var paymentMethodSetup = {
	...
	init : function() {
		...
		$(document).on("click", ".edit-button", function(e) {
			...
			paymentMethodSetup.paymentMethodData.id = $.trim($(tr).find(".col-id")[0].value);
			paymentMethodSetup.paymentMethodData.name = $.trim($(tr).find(".col-name")[0].value);
			paymentMethodSetup.paymentMethodData.remarks = $.trim($(tr).find(".col-remarks")[0].value);
		...
		$(document).on("click", ".delete-button", function(e) {
			...
			if(id > 0) {
				swal({
	            	...
	            	if(e.value) {
	            		paymentMethodSetup.doDelete(_btn, id);
	            	}
	            });
			} else {
				$(tr).remove();
	...
	doCreate : function(_btn) {
		...
		$.ajax({
            ...
            url: paymentMethodSetup.createUrl,
            ...
            success: function (data) {
            	if(data.status) {
            		...
            		Dashmix.helpers('notify', {
                		...
                		message: data.errors && data.errors.length > 0 ? data.errors[0] : 'Failed to create payment method, please try again!',
                		delay: 1e3
        			});
            	}
            },
	...
	doUpdate : function(_btn, id) {
		...
		$.ajax({
			type: "PUT",
            contentType: "application/json",
            url: paymentMethodSetup.updateUrl,
            data: JSON.stringify(serializeForm),
            dataType: 'json',
            timeout: 600000,
            success: function (data) {
            	if(data.status) {
            		...
            		Dashmix.helpers('notify', {
                		...
                		type: 'danger', 
                		icon: 'fa fa-times mr-1', 
                		message: data.errors && data.errors.length > 0 ? data.errors[0] : 'Failed to update payment method, please try again!',
                		delay: 1e3
        			});
            	}
            },
	...
	doDelete : function(_btn, id) {
		...
		$.ajax({
			type: "DELETE",
            contentType: "application/json",
            url: paymentMethodSetup.deleteUrl + id,
            dataType: 'json',
            timeout: 600000,
            success: function (data) {
            	if(data.status) {
            		...
            		Dashmix.helpers('notify', {
                		...
                		type: 'danger', 
                		icon: 'fa fa-times mr-1', 
                		message: data.errors && data.errors.length > 0 ? data.errors[0] : 'Failed to delete payment method, please try again!',
                		delay: 1e3
        			});
            	}
            },
